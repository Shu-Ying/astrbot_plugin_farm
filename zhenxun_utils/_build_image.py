...
class BuildImage:
    ...
    async def resize(self, ratio: float = 0, width: int = 0, height: int = 0) -> Self:
        """
        压缩图片
    ...
    async def crop(self, box: tuple[int, int, int, int]) -> Self:
        """
        裁剪图片
    ...
    async def transparent(self, alpha_ratio: float = 1, n: int = 0) -> Self:
        """
        图片透明化
    ...
    async def rotate(self, angle: int, expand: bool = False) -> Self:
        """
        旋转图片
    ...
    async def transpose(self, angle: Literal[0, 1, 2, 3, 4, 5, 6]) -> Self:
        """
        旋转图片(包括边框)
    ...
    async def filter(self, filter_: str, aud: int | None = None) -> Self:
        """
        图片变化
